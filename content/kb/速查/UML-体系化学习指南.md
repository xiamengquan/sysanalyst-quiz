...
**三大核心元素（必考）**：**对象**、**生命线**、**消息**——缺任一无法完整表达一次交互（2014 上午真题口径）。

| 核心元素 | 含义 | 图中长什么样 |
|----------|------|--------------|
| **对象 Object** | 参与交互的实例或角色（可写 `名称:类名`） | 顶部矩形框；系统外参与者可用 actor（小人） |
| **生命线 Lifeline** | 对象在时间轴上的存在 | 对象框下方的竖虚线；**时间自上而下** |
| **消息 Message** | 对象间按顺序传递的调用/信号 | 生命线之间的水平箭头（同步实线、返回虚线） |

**易混（答卷）**：**泳道**是**活动图**元素（划分职责区域），**不属于**序列图基本构成；选项把「对象+泳道+消息」混进序列图的一律排除。
...
| 元素 | 含义 |
|------|------|
| 激活条 Activation | 生命线上的细矩形，表示对象正在执行操作 |
| 同步消息 | 实心箭头，调用方等待返回 |
| 返回 | 虚线箭头（可省略） |
| 异步消息 | 开放箭头，发送方不必等待 |
