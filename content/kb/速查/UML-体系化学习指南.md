...
| **通信图** | 通信图是按对象链接结构描述对象之间消息交互的行为图（UML 1.x 称协作图）。 | 用于说明「谁与谁相连、在链路上传了哪些带序号的消息」，强调协作结构而非时间轴。 | ★★★★ |
...
| 题型 | 数量 | 元素 | 依据 |
|------|------|------|------|
| **选择题 / 综合知识** | **三大** | 对象、生命线、消息 | 2014 上午第 2 题 |
| **案例简答** | **四大** | **参与者**、对象、生命线、消息 | 2026 上案例分析 |

> 选择题里参与者通常**并入「对象」**（顶框 + 生命线）；案例简答要求**单独列出参与者**，写作用「界定系统边界、作为消息发起/接收源头」。

**四大核心元素（案例简答 · 2026 上）**：

| 元素 | 作用（答卷句） |
|------|----------------|
| **参与者 Actor** | 与系统交互的外部实体（用户、外部设备/系统）；作为消息发起或接收源头，界定系统边界 |
| **对象 Object** | 系统内部执行业务的实例（常写 `对象名:类名`）；消息传递的核心载体 |
| **生命线 Lifeline** | 参与者/对象在时间上的存在；纵向体现**交互先后顺序**（上早下晚） |
| **消息 Message** | 参与者↔对象、对象↔对象间传递的交互指令；带箭头与消息名，串联流程节点 |

**三大核心元素（选择题 · 2014 上）**：**对象**、**生命线**、**消息**——缺任一无法完整表达一次交互。

| 核心元素 | 含义 | 图中长什么样 |
|----------|------|--------------|
| **对象 Object** | 参与交互的实例（含系统外参与者，顶框可为 actor 小人） | 顶部矩形框 |
| **生命线 Lifeline** | 对象在时间轴上的存在 | 对象框下方的竖虚线；**时间自上而下** |
| **消息 Message** | 对象间按顺序传递的调用/信号 | 生命线之间的水平箭头（同步实线、返回虚线） |

**易混（答卷）**：
- **泳道**是**活动图**元素，**不属于**序列图基本构成（2014 干扰项）。
- **激活条**是 UML 教材常讲的第 4 元素，但 **2026 案例四大不含激活**；案例题按「参与者、对象、生命线、消息」答。
...
<figcaption>图 3-3 · 序列图读法：对象（顶框）→ 生命线（竖虚线）→ 消息（箭头）；激活条=正在执行</figcaption>
</figure>

**扩展元素（读图常考，非案例四大 / 非选择三大）**：

| 元素 | 含义 |
|------|------|
| 激活条 Activation | 生命线上的细矩形，表示对象正在执行操作（教材「第四基本元素」，案例简答不单独列） |
| 同步消息 | 实心箭头，调用方等待返回 |
| 返回 | 虚线箭头（可省略） |
| 异步消息 | 开放箭头，发送方不必等待 |
| `alt`/`opt`/`loop` | 分支 / 可选 / 循环片段框 |

---

### 3.3b 通信图 Communication（旧称：协作图）

**答卷·定义**：通信图（Communication Diagram）是按对象之间的链接（Link）结构描述消息交互的行为图；在 UML 1.x 中称为**协作图**（Collaboration Diagram）。  
**答卷·作用**：用于说明某一协作场景中「哪些对象彼此相连、在链路上交换了哪些消息」；消息旁用序号表达先后，**强调对象拓扑与协作关系**，而非垂直时间轴。

**与序列图的关系（答卷）**：通信图与序列图同属**交互图**（Interaction），语义等价、视角不同——同一场景可互相转换；序列图突出时间顺序，通信图突出对象间链接结构。
...
<pre class="uml-ascii">
  ...
  · 对象用矩形（可写 对象名:类名）
  · 对象间实线 = 链接（Link，表示可通信）
  · 箭头旁写「序号:消息名」——序号代替生命线表达顺序
</pre>
<figcaption>图 3-3b · 通信图读法：链接上的带序号消息（与图 3-3 同一场景）</figcaption>
</figure>

| 元素 | 含义 |
|------|------|
| 对象 / 角色 | 矩形框，常写作 `名称:类名` 或仅类名 |
| 链接 Link | 对象间实线，表示二者存在可发消息的连接 |
| 消息 | 链路上的箭头；标注 `序号:消息()`（如 `1.2:校验()`） |
| 序号 | 表达消息先后；可用嵌套序号（`1`、`1.1`、`1.2`）表示调用层次 |

| 对比项 | **序列图** | **通信图（协作图）** |
|--------|------------|----------------------|
| **答卷·侧重点** | 按时间轴看「先发谁、后发谁」 | 按链接看「谁连着谁、链路上有哪些消息」 |
| 时间表达 | 生命线自上而下 | 消息序号（可嵌套） |
| 结构表达 | 较弱（对象横排） | 较强（对象自由排布 + 链接） |
| 旧称 / 现称 | 顺序图 ≈ 序列图 | 协作图 ≈ 通信图（UML 2） |
| 题干信号 | 「调用顺序」「时序」「生命线」 | 「协作关系」「对象链接」「带序号消息」；题干写「协作图」亦指本图 |

**易混（答卷）**：
- 通信图 ≠ 类图：类图是静态结构（类与关系）；通信图是某次协作中的**对象实例**与**消息**。
- 通信图 ≠ 活动图：活动图描述流程步骤/并行；通信图描述对象间消息协作。
- 软考材料若写「协作图」，按 UML 2 答「通信图」即可，二者概念等价。

**何时优先画通信图**：题干强调参与对象有哪些、彼此如何连接、消息挂在哪条链上；或要求与序列图对照转换时。
...
- [ ] 六关系：各能默写「是……」「用于……」  
- [ ] 必考五图：各能默写定义 + 作用  
- [ ] 序列图：选择**三大**（对象、生命线、消息）；案例**四大**（参与者、对象、生命线、消息）  
- [ ] 通信图（协作图）：定义 + 作用；与序列图「时间 vs 链接」对比  
...
**E** 同一「提交订单」场景：序列图与通信图各强调什么？旧称「协作图」对应哪张？

**要点**：B 组合（部分生命周期从属整体）、实心菱在订单侧；C 状态机图；D 见 §2.2.1；E 序列强调时间顺序、通信强调对象链接与带序号消息，协作图=通信图（见 §3.3b）。
...
- 对齐 **OMG UML 2.5.1**。软考或用「顺序图/协作图」旧称，与序列图/通信图概念等价即可（通信图详解见 §3.3b）。  
