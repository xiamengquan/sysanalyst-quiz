...
| 角色 | 定义 | 作用 |
|------|------|------|
| **边界 Boundary** | 系统与参与者/外系统之间的交互接口类 | 收发与适配，隔离内外，不扛核心业务持久状态 |
| **控制 Control** | 协调某一用例（流程）的编排类 | 串步骤、调实体、处理分支；不长期存业务数据 |
| **实体 Entity** | 问题域中有业务意义的信息与规则载体 | 表达并保存「业务是什么」；被控制类读写 |

**口诀**：界面收发、控制编排、实体存业务。  
**协作**：参与者→边界→控制→实体；一条用例常 ≈ 1B + 1C + 若干 E。详解见 `速查/结构化与面向对象分析-体系化学习路径.md` · §4.4。
